...
export const useProducts = (initialProducts, settings, addNotification, user, departments = [], allColors = [], sizes = []) => {
  ...
  const { addProduct: addProductService } = require('@/lib/productsService');
  const addProduct = useCallback(async (productData, imageFiles, setUploadProgress) => {
    // استدعاء دالة الخدمة الموحدة لإضافة المنتج
    const result = await addProductService(productData, imageFiles, setUploadProgress, user, departments, allColors, sizes);
    if (result.success && result.data) {
      setProducts(prev => [result.data, ...prev]);
      if (addNotification) {
        addNotification({
          title: '✅ تم إضافة المنتج بنجاح',
          message: `تم إضافة المنتج "${result.data.name}" بنجاح مع ${result.data.variants?.length || 0} متغير`,
          type: 'success'
        });
      }
    } else if (result.error) {
      toast({ title: 'خطأ في إضافة المنتج', description: result.error, variant: 'destructive' });
    }
    return result;
  }, [settings, user, departments, allColors, sizes, addNotification]);
